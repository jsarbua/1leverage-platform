...
export default function SettingsPage() {
  ...
  const inp={background:"var(--surface2)",border:"1px solid var(--border)",borderRadius:7,padding:"8px 11px",color:"var(--text)",fontSize:13,fontFamily:"var(--font-b)",outline:"none",width:"100%"};
  return (<div>
      ...
      <div className="card">
        ...
        {[{k:"companyName",l:"Company Name"},{k:"hrEmail",l:"HR Escalation Email"},{k:"recruitingEmail",l:"Recruiting Email"},{k:"rankingThreshold",l:"Min Candidate Score (%)"}].map(({k,l})=>(
          <div key={k} style={{display:"flex",flexDirection:"column",gap:5,padding:"13px 0",borderBottom:"1px solid var(--border)"}}>
            ...
            <input style={inp} value={s[k]} onChange={e=>upd(k,e.target.value)}/>
          </div>
        ))}
      </div>
      <div className="card">
